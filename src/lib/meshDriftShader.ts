/**
 * "Mesh drift" fragment shader, from the 21st.dev Shader Builder.
 *
 * Kept verbatim as supplied. The uniform layout is packed deliberately: seven
 * vec4s plus eight colour vec3s comes to 15 fragment uniform vectors, one under
 * WebGL1's guaranteed minimum, so this compiles on the low-end Android hardware
 * a lot of the ad traffic arrives on. The #define block restores readable u_*
 * names on top of the packed vectors — do not "tidy" it into separate uniforms.
 */
