...
**Formular** (`ContactForm.tsx`): progresiv — name → phone+email → budget (€1500-3000 / €3000-5000 / €5000+ / discuss) → submit → `POST /api/contact` → nodemailer SMTP → contact@alcaziurobert.ro. Pe mobil: Drawer prin `ContactCTA`. Backend: `server.js` (Express + nodemailer, singurul backend — servește și `dist/` + fallback-ul SPA). Meta CAPI `Lead` e integrat tot acolo, gated pe env.
...
- [x] Ștergere assets orfane — FĂCUT 2026-08-24 (9), vezi jurnal.
- [x] Componente moarte + deps moarte — FĂCUT 2026-08-24 (9), vezi jurnal.
- [ ] Screenshot nou pentru unbacde10/travel-twin dacă Robert vrea 10 proiecte în grid (acum sunt 8 + featured).
- [x] Scriptul Lovable — Robert a confirmat: nu mai editează prin Lovable, scos din producție 2026-08-24 (9).
- [x] Alinierea prețului de intrare — REZOLVAT 2026-09-04: Robert a decis **fără preț și fără câmp de buget pe site**. Calificarea se face pe apel. Nu reintroduce selectorul de buget descris în §3/§4 sau în versiunile vechi din PRODUCT.md; nu a existat niciodată în formularul livrat.
- [ ] Deciziile Robert rămase: activare CAPI (env + Privacy Policy — notă: GA4 e deja activ condiționat de consimțământ, vezi sesiunea de cookie consent), migrarea .ro→.com.

## 6. JURNAL DE LUCRU

- **2026-09-04 (11)**: **Bug-uri mobile + shader portocaliu + pagina devine funnel.** Pornit de la raportul lui Robert: pe telefon meniul nu acoperea tot ecranul și se vedea o fâșie de site jos, la scroll.

  (a) **Cauza, măsurată în browser, nu presupusă**: `index.css` pune `overflow-x: clip` pe rădăcină, iar regula de propagare din CSS Overflow 3 trimite overflow-ul lui `body` către viewport doar când rădăcina e `visible` pe ambele axe. Nu e — deci `body.style.overflow = "hidden"` din Navbar **nu bloca nimic**. Probă cu PageDown pe pagina reală: fără lock 0→738, cu `body{overflow:hidden}` 0→738, cu `html{overflow:hidden}` 0→0. Pe desktop era mascat de clasa `lenis-stopped` (care pune overflow pe html); pe touch Lenis nu se instanțiază deloc, deci telefonul nu primea blocare de nicăieri. Cele două simptome sunt același defect: pagina se mișcă → bara de browser se retrage → overlay-ul `position:fixed` dimensionat din viewportul de layout rămâne în urmă → în decalaj se vede pagina. De aceea fâșia apărea exact la scroll.

  (b) **`src/hooks/use-scroll-lock.ts`** (nou): blochează `overflow-y` pe `documentElement` — axa x e `clip !important` în CSS-ul critic din `index.html`, deci shorthand-ul și-ar fi pierdut oricum jumătatea aceea — compensează scrollbar-ul pe `body`, oprește Lenis, restaurează poziția la închidere. Folosit de Navbar **și** de `ContactCTA`: Vaul blochează doar pe Safari (`usePositionFixed` iese devreme dacă `!isSafari()`, iar calea mobilă rulează `if (isIOS())`), deci pe Android drawer-ul de contact avea exact același bug, în componenta care aduce lead-uri. Expune `pinBody` (implicit oprit) ca escaladare dacă apare scroll rezidual pe vreun browser real.

  (c) Overlay dimensionat în `dvh` + `env(safe-area-inset-*)`; `100vh`→`100dvh` în drawer, modalul de cookie-uri și cele 5 pagini. A11y găsit la audit: butonul de meniu **nu avea niciun nume accesibil sub 400px** (eticheta e `display:none`, burgerul e `aria-hidden`), `.nav-ov-index` era 4.12:1 pe #121212, butoanele de limbă rămâneau la 32px pe telefon (regula din Navbar bătea prin specificitate regula proprie a componentei), `aria-describedby` trimitea spre un id inexistent pe fiecare câmp — rezolvat prin înregistrare în context, nu citire DOM la render — câmpurile obligatorii n-aveau `aria-required`, iar `index.html` avea title/description în engleză sub `lang="ro"`.

  (d) **Shader**: rețetă nouă `ember` = reglajul lui `cobalt` copiat vector cu vector, cu rampa brandului (#0F0806 → #8A3410 → #ED5C1B → #FFD9A8). Rețeta `brand` avea culorile bune dar reglajul vechi și plat (fără warp, vinietă, blur, cursor), deci nu putea fi activată ca atare fără să se piardă mișcarea. Două abateri deliberate: brightness în jos și vinieta dublată, pentru că accentul din h1 e chiar #ED5C1B și pe un câmp tot portocaliu înceta să mai fie accent — pe albastru se separa gratis.

  (e) **Funnel** (decizii Robert în sesiune: fără preț și fără buget, garanție pe rezultat, doar formular fără calendar, restructurare completă). Ordine nouă: hero → **bandă de dovezi** (cele 3 cifre + logo-uri + linia de raritate) → statement (problema) → **OfferSection** (nouă) → work → testimoniale → proces → compounding → FAQ → contact. Aprinse dovezile care zăceau nerandate: `stats.*` (`StatsBandSection` era neimportat din 2026-08-25 — `CountUp` mutat în banda de dovezi, fișierul șters), `clients.depth_line`, `portfolio.metrics.*` pe carduri. `ANCHOR_IDS` reordonat după pagină + `offer`. Hero primește eyebrow de raritate și bandă de încredere, redusă la garanție sub 380px lățime sau 680px înălțime ca CTA-ul să rămână above the fold pe 360×640 — verificat, 511px pe un ecran de 640.

  (f) **Formularul, ca CTA principal**: la succes nu mai apare un toast care dispare în 3 secunde în timp ce drawer-ul se închide singur — apare un **ecran de succes** care înlocuiește formularul, drawer-ul rămâne deschis, vizitatorul îl închide. Conține ce urmează, garanția repetată exact când omul s-a angajat, o cifră de dovadă și WhatsApp pentru nerăbdători; header-ul drawer-ului se schimbă odată cu el. Autofill completat: `inputMode`/`autoCapitalize`/`autoCorrect`/`enterKeyHint` pe telefon.

  (g) Disciplina CTA: textele care promiteau programare fără să existe calendar au fost rescrise; CTA-urile `mailto:` din pagina de studii de caz trec acum prin `ContactCTA` (scoteau vizitatorul din funnel, formularul și backup-ul de lead-uri nu-l vedeau niciodată); formularul trece primul în FAB-ul mobil. FAQ rescris de la 3 afirmații deghizate în întrebări la **6 obiecții reale**, plus **`FAQPage` JSON-LD** generat din aceleași chei. Namespace-uri moarte șterse (`hero_v2`, `difference`, `frictionless`). Placeholder-ele `minHeight` remăsurate în browser la 390×844 și 1440×900 — `#work` rezerva 1600px pentru ~730px reali. Prerender: textul homepage-ului a crescut de la 7306 la 9555 caractere, deci oferta și FAQ-ul ajung la crawlere.

  **Gotcha pentru sesiunile viitoare**: nu pune backtick-uri în comentariile CSS din blocurile `<style>` cu template literal — închid string-ul, iar tsc raportează erori de JSX cu 200 de linii mai jos, în alt fișier ca aparență. Și: `.env.local` are 2 chei SMTP goale, deci `/api/contact` răspunde 500 `smtp_not_configured` local; lead-ul **se salvează totuși** în `leads.jsonl` (plasa de siguranță ține), dar testul real de trimitere nu se poate face local fără credențiale — ecranul de succes a fost verificat interceptând răspunsul.

- **2026-08-25 (10)**: **Redesign navbar + carusel portofoliu + curățenie de platformă.** (a) Navbar rescris: bară minimală (logo + EN/RO + "Menu" + CTA) și meniu fullscreen prin portal, cu Escape/focus-trap/scroll-lock; cauza textului invizibil din header era `MadeByHumans` care redefinea `--ink` pe `:root` — tokenii sunt acum scopați pe secțiune. (b) Contrast urcat pe tot site-ul dark (alfa 0.38→0.60 … 0.85→0.92, `#9CA3AF`→`#C4C9D0`, placeholder `white/35`→`white/50`, `--muted-foreground` 65%→74%); reparate `.faq-content` (#1f1f1f pe negru) și `.faq-cta-link` (#262626 → alb, acum duce pe WhatsApp `wa.me/40773858164`). (c) `StatsBandSection` scoasă din pagină. (d) `SelectedWorkSection` rescrisă: carusel infinit cu auto-scroll + drag pe desktop (wrap măsurat din layout, nu `scrollWidth/2`), grilă de wordmark-uri pe 2 coloane sub 768px; adăugate `portfolio.categories.*`, `work.intro`, `work.explore`. (e) Logo nou din `public/logo.png`: `logo-mark.webp` regenerat (222×128), favicon set complet (ico 16/32/48 + PNG 32/48/192/512 + apple-touch pe fundal alb), loader trecut pe fundal `#0F0F0F`, `theme-color` `#0F0F0F`. (f) Fundalul din spatele cardului de footer: `#FAF8F6` → `#0F0F0F`. (g) **Scoasă toată dependența de PaaS**: șterse `api/contact.ts` (funcție Vercel), `vercel.json` și `@vercel/node`; headerele de securitate din `vercel.json` și evenimentul Meta CAPI au fost portate în `server.js`; `bun.lockb` șters (npm e singurul lockfile); `package.json` redenumit `vite_react_shadcn_ts` → `alcaziurobert-ro` + `engines.node >=20`; scos "Vercel" din JSON-LD; `.env.example` rescris pentru cPanel/Node; workflow-ul primește `VITE_GA_MEASUREMENT_ID` la build. Deploy-ul rămâne cel existent: GitHub Actions → FTPS → Hosterion (cPanel → Setup Node.js App, restart prin `tmp/restart.txt`). (h) **Footer rescris**: scos cardul cu mesh portocaliu (și `LiquidMesh.tsx` + dependența `@paper-design/shaders-react`, singurul consumator); structură nouă pe desktop — wordmark uriaș "ALCAZIU ROBERT" full-bleed (SVG cu `textLength`, se potrivește pe orice lățime), rând pitch + CTA / logo / contact (email, telefon, WhatsApp), hairline, linkuri legale și copyright; pe mobil totul stivuit și centrat, rânduri de contact full-width cu tap target ≥56px și padding de 92px jos ca FAB-ul să nu acopere copyright-ul. Verificat la 360/390/768/1440, zero overflow orizontal.

- **2026-08-24 (9)**: **Optimizare completă de viteză + curățenie cod/assets mort** (Robert: "optimizează absolut tot, aruncă ce nu e folosit"). Audit fresh cu 3 explorări paralele (nu s-a preluat orbește lista veche din §3 — codul se schimbase între timp cu cookie consent/GA4/FAQ rewrite). Rezultate: (a) **17 componente moarte șterse** (Features, GlobeSection, PainpointsSection, ManifestoSection, TechStackSection, WhyCloudflare, CFFortuneSection, IntroLanding, CadastreGrid, BrandMarks, ScrollToTop, logo-cloud, infinite-slider, feature-section, decor-icon, header/mobile-nav/portal/logo) + `src/lib/cobe/`; (b) **37 primitive shadcn nefolosite șterse** din `ui/` (alert-dialog, avatar, calendar, card, carousel, chart, checkbox, command, dropdown-menu, sidebar, table, tabs, textarea, toggle etc. — lista completă în istoricul sesiunii); (c) **29 dependențe npm dezinstalate** (lottie-react, gsap, @gsap/react, date-fns, recharts, react-day-picker, react-resizable-panels, cmdk, embla-carousel-react, input-otp, react-use-measure, @tanstack/react-query + 18 pachete @radix-ui/react-*), inclusiv scoaterea `QueryClientProvider`-ului mort din `App.tsx` (avea zero query-uri) — `vite.config.ts` `manualChunks` actualizat (scos `react-use-measure` din `motion-vendor`); (d) `tailwind.config.ts` + `src/index.css`: șters aliasurile moarte `brockmann`/`playfair`, scala de culoare `pulse` (umbrită de `.nav-link` local din Navbar), gradient-urile `hero-gradient`/`hero-gradient-2`/`pulse-gradient`, animațiile `fade-in-right`/`fade-in-left`/`float`; `src/App.css` șters (neimportat); (e) **8 namespace-uri locale moarte** șterse din `en.json`+`ro.json` (painpoints, cf_fortune, tech_stack, globe, roadmap, whyCf, statement, case_study — verificate individual cu grep înainte de ștergere); (f) **32 assets orfane din public/ șterse (~2.64MB)** + `CF-Logo 1.png`/`tiktok-logo (1).png` din rădăcină + `dist/` local; (g) **5 imagini live recomprimate** (picaps3.webp 218→118KB, Header-background.webp 245→32KB — doar og:image, lukton.webp 168→65KB, alma.webp 156→48KB, everati.webp 155→94KB — redimensionate la max 1200px lățime, suficient pentru cardurile grid); (h) **font "General Sans" self-hosted**: descărcate cele 4 weight-uri (400/500/600/700 woff2) direct de la Fontshare în `public/fonts/`, `@font-face` cu `font-display: swap` în `index.css`, scoase cele 3 tag-uri Fontshare (2× preconnect + stylesheet blocant) din `index.html`, înlocuite cu un singur `<link rel="preload">` pe weight-ul 500; (i) **scriptul Lovable `gptengineer.js` scos din producție** (Robert a confirmat că nu mai editează prin Lovable) — `lovable-tagger`/`componentTagger()` din `vite.config.ts` rămâne neatins (dev-only). Rezultat măsurat: bundle JS principal 160.96KB→129.11KB (gzip 53.77→43.19KB), CSS 78.41KB→41.84KB (gzip 13.85→8.24KB), `node_modules` cu 91 pachete mai puțin, `public/` de la referențiate+orfane combinate la 973KB total. Verificat: `npx tsc --noEmit` ✓, `npm run lint` ✓ (0 erori noi — singura eroare rămasă, `require()` în `tailwind.config.ts`, e preexistentă și în afara scope-ului), `npm run build` ✓ după fiecare bloc mare de modificări, zero referințe rupte (grep exhaustiv pe toate fișierele șterse, zero hit-uri rămase în `src/`).
